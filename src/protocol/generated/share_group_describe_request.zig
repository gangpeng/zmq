// Auto-generated from ShareGroupDescribeRequest.json
// API Key: 77, Type: request
// Valid Versions: 0, Flexible Versions: 0+
//
// DO NOT EDIT - generated by codegen_v2.py

const std = @import("std");
const ser = @import("../serialization.zig");
const Allocator = std.mem.Allocator;

pub const ShareGroupDescribeRequest = struct {
    /// The ids of the groups to describe
    /// Versions: 0+
    group_ids: []const ?[]const u8 = &.{},
    /// Whether to include authorized operations.
    /// Versions: 0+
    include_authorized_operations: bool = false,

    pub fn serialize(self: *const ShareGroupDescribeRequest, buf: []u8, pos: *usize, _: i16) void {
        ser.writeCompactArrayLen(buf, pos, self.group_ids.len);
        for (self.group_ids) |item| {
            ser.writeCompactString(buf, pos, item);
        }
        ser.writeBool(buf, pos, self.include_authorized_operations);
        ser.writeEmptyTaggedFields(buf, pos);
    }

    pub fn deserialize(alloc: Allocator, buf: []const u8, pos: *usize, _: i16) !ShareGroupDescribeRequest {
        var result = ShareGroupDescribeRequest{};
        const group_ids_len: usize = (try ser.readCompactArrayLen(buf, pos)) orelse 0;
        if (group_ids_len > 0) {
            const group_ids_items = try alloc.alloc(?[]const u8, group_ids_len);
            for (group_ids_items) |*item| {
                item.* = try ser.readCompactString(buf, pos);
            }
            result.group_ids = group_ids_items;
        }
        result.include_authorized_operations = try ser.readBool(buf, pos);
        try ser.skipTaggedFields(buf, pos);
        return result;
    }

    pub fn calcSize(self: *const ShareGroupDescribeRequest, _: i16) usize {
        var size: usize = 0;
        size += ser.unsignedVarintSize(self.group_ids.len + 1);
        for (self.group_ids) |item| {
            size += ser.compactStringSize(item);
        }
        size += 1;
        size += 1;
        return size;
    }
};
