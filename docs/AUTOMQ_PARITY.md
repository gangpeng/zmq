...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  retry, fetch storage errors, and compaction orphan retry. Remaining durability
  work is live MinIO/provider crash-fault recovery, deeper multipart fault
  injection, and crash/fault recovery against quorum/controller metadata.
