...
| Consumer groups/transactions | Partial. Core flows exist with simplified persistence; OffsetDelete and TxnOffsetCommit now flush offset mutations, and InitProducerId now flushes producer allocations, to local metadata snapshots instead of relying on clean shutdown. | Kafka-compatible rebalances, offset lifecycle, transactions, fencing, and coordinator failover. |
