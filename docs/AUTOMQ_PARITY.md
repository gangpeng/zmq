...
- Replace single-node no-op inter-broker handlers with real controller-backed
  ...
  metadata now moves tracked partitions from timed-out fenced brokers to the
  surviving broker instead of treating reassignment as a no-op. Controller
  metadata full snapshot records now preserve broker/PID state across Raft log
  compaction.
