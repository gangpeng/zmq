...
- Controller-style KV/node/router/license/node-id/group mutations now have a
  ...
  router metadata, node registry, license, node-id allocator, and stream
  create/prepare/commit/open/close/trim/delete metadata survive leader kill,
...
- Replace single-node no-op inter-broker handlers with real controller-backed
  ...
  node registry, license, node-id allocator, and stream
  create/prepare/commit/open/close/trim/delete metadata survive controller
  leader kill, replacement-leader mutation, and old-leader restart/rejoin.
