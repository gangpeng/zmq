...
- Add performance baselines for produce/fetch throughput, p99 latency, S3
  ...
  harness only when `ZMQ_RUN_BENCH_COMPARE=1` is set, and the default suite
  self-tests its target parsing and ratio formatting. Remaining work: CI
  execution of the comparative gate and broader live comparative performance
  profiles.
