...
| Kafka broker APIs | Partial. 79 advertised APIs; many handlers are simplified single-node semantics. Auto-created topics and CreateTopics write topic snapshots before exposing topic creation and roll back local topic/partition visibility when the shared snapshot write fails; CreateTopics also applies supported numeric configs at creation time, rejects invalid `min.insync.replicas`/`max.message.bytes` values before topic visibility, rejects manual assignments outside current single-node semantics, and rolls back visible topic/partition/ObjectManager state when local `topics.meta` or object snapshots cannot be written after the shared snapshot. Produce now returns per-partition storage errors instead of successful acknowledgements when post-append local partition-state, ObjectManager, or producer-sequence checkpoints cannot be written. CreatePartitions likewise rejects manual assignments outside current single-node semantics, writes topic snapshots before acknowledging partition-count expansion, and rolls back local partition/ObjectManager visibility when shared or local topic/ObjectManager snapshots cannot be written. AlterConfigs and IncrementalAlterConfigs roll back topic config visibility when shared or local topic/ObjectManager snapshots cannot be written; DeleteTopics writes topic snapshots before acknowledging deletion, defers local partition, share-state, and replica-directory cleanup until the shared snapshot succeeds, persists side-state cleanup across local restart, rolls back topic visibility without dropping those side states when the shared snapshot write fails, and reports storage errors when post-snapshot local cleanup checkpoints fail. DeleteRecords now rolls back visible partition start offsets and ObjectManager trim metadata and returns per-partition storage errors when local partition-state or object snapshots cannot be written. Internal metadata-log snapshot writers for `__cluster_metadata`, `__consumer_offsets`, and `__transaction_state` now propagate partition-state checkpoint failures so request handlers fail closed instead of acknowledging records that may not replay after restart. ConsumerGroupDescribe, DescribeLogDirs, partition reassignment APIs, AlterClientQuotas, AlterUserScramCredentials, DescribeClientQuotas, DescribeCluster, DescribeQuorum, DescribeUserScramCredentials, ElectLeaders, DescribeProducers, DescribeTopicPartitions, DescribeTransactions, GetTelemetrySubscriptions, ListClientMetricsResources, ListTransactions, PushTelemetry, and UpdateFeatures now decode generated requests and return request-scoped generated responses instead of blanket hand-encoded results. | Full schema-valid decode/encode and Kafka-compatible semantics for every advertised version. |
...
- Status: Source-level protocol audit tests now parse the generated index and
  ...
  return terminal leave responses and join/rejoin error responses without
  scheduling another heartbeat, and map stale member epochs/static instance
  ...
  resources/results.
  CreateTopics, AlterConfigs, and IncrementalAlterConfigs now validate
  ...
  `min.insync.replicas`, and `min.insync.replicas <= replication.factor`.
...
- Controller-style KV/node/router/license/node-id/group mutations now have a
  ...
  bad-credential, bad-TLS-trust, and ACL-denied-produce real-client probes in
  addition to default metadata/produce/fetch/offset checks. Remaining gap:
  broaden OAuth negative-case semantic client fixtures.
...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  local topic metadata snapshots, and topic IDs, partition counts, supported
  ...
  durability work is provider-specific multipart faults and live provider
  outage/fault recovery against quorum/controller metadata.
...
- Replace single-node no-op inter-broker handlers with real controller-backed
  ...
  Broker registration/unregistration state, broker rack metadata, and
...
- Add compatibility runs with Java, librdkafka, Go, Python, and Kafka CLI
  ...
  bootstrap endpoints. `ZMQ_CLIENT_MATRIX_SEMANTICS` can require real-client
  ...
  bad-credential, bad-TLS-trust, and ACL-denied-produce fail-closed checks over
  those secured-client profiles. Remaining work: add OAuth negative-case
  semantic client fixtures and expand live secured-client CI profiles.
- Add chaos tests for SIGKILL, network partition, S3 outage, clock skew, and
  ...
  inject network partitions. Remaining work: broader
  multi-broker/controller partition matrices and live provider outage runs.
- Add performance baselines for produce/fetch throughput, p99 latency, S3
  ...
  growth with the project's tracking allocator. Gated `bench-compare` now runs
  the existing ZMQ/Kafka/AutoMQ comparison harness only when
  `ZMQ_RUN_BENCH_COMPARE=1` is set, and the default suite self-tests its target
  parsing and ratio formatting. Remaining work: live MinIO/provider
  performance trend thresholds and CI execution of the comparative gate.
