...
| Security | Partial. TLS, SASL, OAuth, SCRAM, and ACL pieces exist. Advertised broker APIs now have default-suite ACL resource/operation mapping coverage, excluding pre-auth SASL frames, ACL admin, selected cluster/transaction introspection, and simple transaction coordinator authorization denials are covered with generated response shapes, and unsupported-version rejection now runs before ACL denial response construction. | Interop suites, negative tests, cert rotation, and API-specific authorization response-shape coverage for the remaining advertised APIs. |
