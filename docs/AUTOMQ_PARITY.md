...
- Add chaos tests for SIGKILL, network partition, S3 outage, clock skew, and
  ...
  `ZMQ_KRAFT_NETWORK_UP`, or scheduled named controller/broker partition phases
  with `ZMQ_KRAFT_NETWORK_MATRIX` and `ZMQ_KRAFT_NETWORK_<PHASE>_{DOWN,UP,EXPECT}`
  overrides. Remaining work: live provider outage CI profiles and broader
  environment coverage for those scheduled matrices.
