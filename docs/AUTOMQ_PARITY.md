...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  broker tick now enforces each group's configured session timeout instead of a
  global timeout;
  ...
  ports. Remaining durability work is atomic transaction-marker log/snapshot
  commits, broader live provider matrix coverage, provider-specific multipart
