...
| Kafka generated schemas | Broad. 110 request schemas generated. Default-value serialize/deserialize/calcSize smoke tests now cover all 230 generated top-level request/response/header/record types across common protocol versions 0-20. Representative non-default golden wire fixtures now cover generated request headers plus MetadataRequest, ProduceRequest, ProduceResponse v10 and FetchResponse v16 KIP-951 tagged `CurrentLeader`/`NodeEndpoints` metadata, Vote/BeginQuorumEpoch/EndQuorumEpoch v1 KIP-853 tagged `NodeEndpoints`, FetchSnapshot request/response v1 KIP-853 tagged `ClusterId`/`ReplicaDirectoryId`/`CurrentLeader`/`NodeEndpoints`, BrokerHeartbeatRequest v1 tagged `OfflineLogDirs`, ApiVersionsResponse, and CreateTopicsRequest across legacy and first-flexible encodings. | Extend non-default field fixtures, min/max/first-flexible semantic vectors, and golden wire fixtures to every generated request/response pair. |
