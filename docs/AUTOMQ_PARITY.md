...
- Validate rack-aware routing and auto-balancer decisions under load. Status:
  ...
  on stale plans without partial mutation, no-ops without active targets, and
  ...
  convergence and old-owner write fencing. Cross-broker target topic/data
  convergence plus automatic load/scale orchestration gates remain.
