...
- AlterReplicaLogDirs is now advertised and handled through generated request
  and response schemas. Delegation-token APIs 38-41 are advertised with strict
...
join/assignment/heartbeat continuity, and KIP-848 ConsumerGroupDescribe
member/subscription/assignment visibility, through controller leader
kill/election, old-leader fresh rejoin, surviving-controller restart, and broker
restart. It now also sends live grouped OffsetFetch v8 requests covering a
...
unknown member, and stale member epoch.
...
`api_support.zig` and now probed on the broker port to fail closed before body
decode.
