...
| P1 | Stateless broker replacement | A replacement broker can rebuild state from shared storage and quorum metadata without local disk metadata or manual repair. | In progress. S3/object repair exists, committed offsets are now written to versioned `__consumer_offsets` records and replayed from recovered S3 WAL objects, consumer group lifecycle snapshots are now written to `__consumer_offsets` and replayed from S3 WAL, and the combined controller+broker failover gate now restarts the killed AutoMQ leader after deleting its local data dir and verifies it rebuilds quorum-backed AutoMQ metadata from Raft. Broader broker-only replacement, topic configs, transaction metadata, and fail-closed group snapshot durability remain. |
...
| Stateless brokers | Partial. Combined AutoMQ metadata can rejoin from a fresh local data dir through quorum replay, local/S3 object repair covers data-path replacement, and committed offsets plus consumer group lifecycle snapshots can rebuild from `__consumer_offsets` S3 WAL records after fresh-dir broker replacement. Local cache/state still has single-node assumptions for topic configs, transactions, and broader broker-only metadata. | Rebuild all broker state from shared storage/controller metadata without data loss or manual repair. |
