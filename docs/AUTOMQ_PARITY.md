...
- Status: Source-level protocol audit tests now parse the generated index and
  ...
  supported features, fails closed with rollback when local persistence fails,
  persists it across local restart, and exposes supported and finalized features
  through ApiVersions v3+ tagged fields.
...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  WAL during fresh-dir replacement; Produce now returns per-partition storage
