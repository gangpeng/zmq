...
| Consumer groups/transactions | Partial. Core flows exist with simplified persistence; OffsetDelete, DeleteGroups, and TxnOffsetCommit now flush offset mutations, JoinGroup/SyncGroup/LeaveGroup/DeleteGroups now flush local group lifecycle snapshots, LeaveGroup v3+ can resolve static members by `group_instance_id`, InitProducerId flushes producer allocations, transaction snapshots retain registered partitions, and idempotent producer sequence updates flush after Produce instead of relying on clean shutdown. | Kafka-compatible rebalances, offset lifecycle, transactions, fencing, and coordinator failover. |
