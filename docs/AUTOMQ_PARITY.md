...
| Consumer groups/transactions | Partial. Core flows exist with simplified persistence; OffsetDelete and TxnOffsetCommit now flush offset mutations, InitProducerId flushes producer allocations, transaction snapshots retain registered partitions, and idempotent producer sequence updates flush after Produce instead of relying on clean shutdown. | Kafka-compatible rebalances, offset lifecycle, transactions, fencing, and coordinator failover. |
