...
- Controller-style KV/node/router/license/node-id/group mutations now have a
  ...
  now require admin, consumer-group, transactional, and secured-client
  real-client probes in addition to default metadata/produce/fetch/offset
  checks. Remaining gap: broaden rebalance and security negative-case semantic
  client fixtures.
...
- Add compatibility runs with Java, librdkafka, Go, Python, and Kafka CLI
  ...
  transactional produce paths for confluent-kafka and Java kafka-clients, and
  secured-client runs for kcat, Kafka CLI, kafka-python, confluent-kafka, and
  Java kafka-clients. Remaining work: add deeper rebalance and ACL/security
