...
- AlterReplicaLogDirs is now advertised and handled through generated request
  ...
  using the broker registration epoch and configured directory ID, and
  ...
  DescribeConfigs read-back, and validate-only creation checks that leave the
  probe topic absent.
  ...
  checks, and exact finalized feature-level visibility.
...
- S3 WAL/object storage paths exist, but full AutoMQ S3Stream lifecycle
  ...
  peer endpoints. Startup now also fails closed on unreadable config files,
  invalid cluster-critical integer settings, negative node IDs, malformed
  `process.roles`, missing CLI flag values, and unknown CLI arguments instead
  of silently falling back to default node identity or listener ports.
...
in its AutoMQ metadata scenario.
...
broker-invalid positive OAuth tokens before execution, and missing-exp JWTs are
pinned as valid OAuth-negative vectors. Java/Kafka CLI OAUTHBEARER JAAS
...
semantics remain wall-clock based, including record timestamps, token/session
expiry, S3 signing, controller/group heartbeat snapshots, and transaction start
timestamps.
...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  LogCache and broker-level fetch after restart, and malformed filesystem WAL
  segment names plus malformed or corrupt partition WAL records fail closed
  during broker open instead of being skipped; existing unreadable or malformed
  ...
  registration fails; replacement
  ...
  records so broker offset rewrites preserve payload values.
...
- Replace single-node no-op inter-broker handlers with real controller-backed
  ...
  voter changes, broker lifecycle calls, and ControllerRegistration.
...
- Validate rack-aware routing and auto-balancer decisions under load. Status:
  ...
  clean three-node Docker E2E gate was re-executed successfully with `49/49`
  passing, including cross-node fetch, dynamic source reassignment,
  target-owner Produce/Fetch after reassignment, a required fixture-backed
  `load` phase, and a separate required `scale-in` phase inferred from
  ...
  including required-phase inference, and required-phase validation are now
  pinned by `test-e2e`; broader CI execution across real scale-in/out/load
...
- Add compatibility runs with Java, librdkafka, Go, Python, and Kafka CLI
  ...
  tool lacks the required positive or negative security fixture. Deterministic
  ...
  cannot satisfy OAuth coverage while missing-exp negative tokens can. Java and
...
- Add chaos tests for SIGKILL, network partition, S3 outage, clock skew, and
  ...
  can infer phases from the required phase list. Those hooks now receive the
  active topic plus broker, controller, metrics, container, and MinIO context,
  with deterministic default-suite self-test coverage.
...
- Add performance baselines for produce/fetch throughput, p99 latency, S3
  ...
  `ZMQ_BENCH_LIVE_S3_MAX_REQUESTS_PER_MIB`. The local benchmark gate has been
