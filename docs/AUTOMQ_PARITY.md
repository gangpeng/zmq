...
| Tests | Improving. Unit/integration tests run under Zig 0.16. Controller, broker registry, and metadata client tests are included in the default suite. `zig build bench` now exercises repeatable local produce/fetch, S3 WAL request-volume, recovery-time, and bounded memory-growth performance gates. Gated `test-chaos` now covers broker SIGKILL/local-WAL restart, slow/partial client frames, far-future client timestamps, sync S3 WAL outage fail-closed behavior, CI-provided network-partition hooks, and CI-provided live-S3 outage/heal hooks; gated `test-kraft-failover` can now run CI-provided controller/broker network-partition hooks before failover/restart sequencing. Deterministic fixture self-tests run in the default suite. | Add protocol golden, broader multi-node e2e, broader multi-broker chaos, comparative perf, and expanded client matrix gates. |
...
- Replace single-node no-op inter-broker handlers with real controller-backed
  ...
  broker/controller convergence, verify configured produce behavior during the
  partition, heal the cluster, reconverge controllers, and verify broker data
  continuity before the leader-kill/restart sequence.
...
- Add compatibility runs with Java, librdkafka, Go, Python, and Kafka CLI
  ...
  bad-credential, bad-OAuth-token/JAAS/config, bad-TLS-trust, and
  ACL-denied-produce fail-closed checks over those secured-client profiles.
  Remaining work: expand live secured-client CI profiles.
- Add chaos tests for SIGKILL, network partition, S3 outage, clock skew, and
  ...
  `ZMQ_S3_<PROFILE>_RUN_LIVE_OUTAGE=1`, and `test-kraft-failover` can require
  controller/broker network-partition hooks with `ZMQ_KRAFT_NETWORK_DOWN` and
  `ZMQ_KRAFT_NETWORK_UP`. Remaining work: scheduled multi-broker/controller
  partition matrices and live provider outage CI profiles.
- Add performance baselines for produce/fetch throughput, p99 latency, S3
  ...
  least one Kafka/AutoMQ baseline result for release-gate execution, and
  ...
  target parsing, ratio formatting, threshold parsing, missing-baseline
  detection, throughput/latency regression detection, and error-rate
  enforcement. Remaining work: CI execution of the comparative gate and broader
  live comparative performance profiles.
