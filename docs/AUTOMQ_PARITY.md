...
- Status: Source-level protocol audit tests now parse the generated index and
  ...
  AssignReplicasToDirs key 73 remains non-advertised until full multi-directory
  JBOD movement semantics exist, but direct requests now
  ...
  stable local replica-directory UUID through BrokerRegistration v2, persist and
  replay registered directory IDs through controller Raft metadata, validate
