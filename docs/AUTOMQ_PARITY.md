...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  local offsets/groups when the shared tombstone write fails; JoinGroup,
  LeaveGroup, and SyncGroup now restore the previous local group snapshot when
  their shared lifecycle snapshot write fails; client quota
