...
| Kafka broker APIs | Partial. 67 advertised APIs; many handlers are simplified single-node semantics. ElectLeaders now decodes the generated request and returns per-request partition results instead of blanket success. DescribeProducers now decodes the generated request and scopes active-producer results to requested topic partitions. | Full schema-valid decode/encode and Kafka-compatible semantics for every advertised version. |
