...
- Status: Source-level protocol audit tests now parse the generated index and
  ...
  flexible requests, rejects malformed frames, returns request-scoped
  `snapshot_not_found` until snapshot transfer is implemented, and includes
  v1 current-leader/node-endpoint metadata from committed voter endpoints.
