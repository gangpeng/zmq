...
| P1 | Multi-node KRaft and broker lifecycle | Broker registration, heartbeat, fencing/unfencing, controller failover, rolling restart, and leader epoch behavior are covered by three-node tests. | In progress. Controller Vote, BeginQuorumEpoch, EndQuorumEpoch, DescribeQuorum, BrokerRegistration, BrokerHeartbeat, AllocateProducerIds, AddRaftVoter, and RemoveRaftVoter now use generated schemas on the controller port; controller tests are part of the default Zig test suite. Gated `test-kraft-failover` now starts three controller-only processes plus a broker-only process, kills the discovered controller leader, verifies surviving controllers elect a replacement, and verifies the broker can produce before and after controller failover. Broker registrations now append controller metadata records, followers persist replicated AppendEntries, controller startup replays durable broker registration records from Raft, and promoted followers can heartbeat brokers without forced re-registration. Broker rolling restart, producer-id durability, and full controller snapshots remain. |
...
| Controller/KRaft | Partial. Local Raft/controller scaffolding exists. Controller ApiVersions and quorum/lifecycle RPC framing now use generated schemas for ApiVersions, Vote, BeginQuorumEpoch, EndQuorumEpoch, DescribeQuorum, BrokerRegistration, BrokerHeartbeat, AllocateProducerIds, AddRaftVoter, and RemoveRaftVoter; telemetry keys 71/72 are no longer treated as voter APIs. Controller tests now run in the default Zig suite and cover malformed Vote plus internal AppendEntries compatibility. A gated controller/broker failover harness exists and broker-only nodes start fenced until controller heartbeat succeeds. Broker registrations are now stored as controller metadata records and replayed from persisted Raft logs on follower promotion/restart. | Add producer-id durability, durable controller snapshots beyond registration records, rolling restart, and broader broker+controller failover gates. |
...
- Replace single-node no-op inter-broker handlers with real controller-backed
  ...
  failover, leader kill, replacement leader convergence, and broker produce
  after failover. Broker registration state now has a Raft-backed controller
  metadata record; followers persist replicated AppendEntries and promoted or
  restarted controllers replay registered brokers before accepting heartbeats.
  Remaining gap: producer-id durability, full controller snapshots, broker
  rolling restart, and broader failover gates.
