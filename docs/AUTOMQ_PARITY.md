...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  WriteTxnMarkers S3 WAL success now flushes marker control batches and the
  updated transaction snapshot in one shared WAL object before advancing local
  state; broker tick now enforces each group's configured session timeout
