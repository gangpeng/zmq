...
| Kafka generated schemas | Broad. 110 request schemas generated. Default-value serialize/deserialize/calcSize smoke tests now cover all 230 top-level request/response/header/record types across common protocol versions 0-20. Representative non-default golden wire fixtures now cover generated request headers plus MetadataRequest, FetchRequest v17 tagged `ClusterId`/`ReplicaState`/`ReplicaDirectoryId`, ProduceRequest, ProduceResponse v10 and FetchResponse v16 KIP-951 tagged `CurrentLeader`/`NodeEndpoints` metadata, Vote/BeginQuorumEpoch/EndQuorumEpoch v1 KIP-853 tagged `NodeEndpoints`, FetchSnapshot request/response v1 KIP-853 tagged `ClusterId`/`ReplicaDirectoryId`/`CurrentLeader`/`NodeEndpoints`, BrokerHeartbeatRequest v1 tagged `OfflineLogDirs`, BrokerRegistration v2 UUID/listener/feature/log-dir metadata, BrokerRegistrationResponse and BrokerHeartbeatResponse lifecycle fields, ControllerRegistration v0 listener/feature metadata, Add/Remove/UpdateRaftVoter v0 endpoint/directory/KRaft-version metadata, Describe/AlterClientQuotas v1 compact entity and operation arrays including nullable default entity names and f64 values, Describe/AlterUserScramCredentials v0 mechanism/iteration/result arrays and compact salt/password bytes, Describe/Create/DeleteAcls v2 filters, creations, nested matching ACLs, and per-result errors, OffsetForLeaderEpoch v4 leader-epoch/end-offset fields, DeleteRecords v2 partition low-watermark fields, SaslHandshake v1 mechanism arrays and SaslAuthenticate v2 compact auth bytes/session lifetime, CreateTopicsResponse v5+ tagged `TopicConfigErrorCode`, UpdateMetadataRequest v8 tagged `Type` plus legacy upper-bound fields, ApiVersionsResponse, transaction coordinator request/response shape transitions, FindCoordinator v2/v4 single/batched lookup encodings, ListOffsets v0 old-style offsets and v7 flexible leader-epoch offsets, ListTransactions v1 request/response filters/results, AutoMQ Get/Put/DeleteKVs v0 compact key/value arrays with nullable bytes, GetNextNodeId v0 cluster/node fields, Update/DescribeLicense v0 compact strings, AutoMQ stream lifecycle, object lifecycle, opening-stream, and describe-stream fixtures across v0/v1 shape transitions, and CreateTopicsRequest across legacy and first-flexible encodings. | Extend non-default field fixtures, min/max/first-flexible semantic vectors, and golden wire fixtures to every generated request/response pair. |
