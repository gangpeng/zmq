...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  EndTxn and WriteTxnMarkers S3 WAL success now flush marker control batches
  and the updated transaction snapshot in one shared WAL object before advancing
  local state; broker tick now enforces each group's configured session timeout
  ...
  atomic EndTxn and WriteTxnMarkers S3 WAL objects now restore both marker
  partition offsets and completed transaction snapshots after fresh-dir
  replacement;
