...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  advancing offsets/HW/cache or retaining duplicate pending entries; fetch returns
  ...
  retry, fetch storage errors, and compaction orphan retry; S3Client multipart
  fault tests now cover missing/malformed part ETags, abort-after-part failure,
  and abort-after-complete failure; a gated `test-minio` build step now covers
  live object round-trip, multipart round-trip, and S3 WAL metadata rebuild plus
  fetch against MinIO/S3. Remaining durability work is broker-process
  crash/restart recovery, broader provider matrix coverage, provider-specific
