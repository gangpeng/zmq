...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  produce/rebuild/resume against MinIO/S3. Remaining durability work is an
  external broker-process crash/restart harness, broader provider matrix
  coverage, provider-specific multipart faults, and crash/fault recovery against
