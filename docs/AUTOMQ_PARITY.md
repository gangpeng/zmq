...
APIs. It now also produces an idempotent batch before failover and verifies
DescribeProducers visibility for that producer through the same checkpoints,
including broker restart, next-sequence progress, and epoch-bump recovery. It now
also sends live grouped OffsetFetch v8 requests covering a
