...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  now writes `__consumer_offsets` tombstones before local removal, replays those
  tombstones during fresh-dir replacement, and preserves local offsets when the
  shared tombstone write fails; client quota
