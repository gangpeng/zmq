...
| Consumer groups/transactions | Partial. Core flows exist with simplified persistence; OffsetDelete now flushes deleted offsets to the local metadata snapshot instead of relying on clean shutdown. | Kafka-compatible rebalances, offset lifecycle, transactions, fencing, and coordinator failover. |
