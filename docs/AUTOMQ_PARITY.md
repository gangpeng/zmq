...
- Replace single-node no-op inter-broker handlers with real controller-backed
  ...
  the built-in Docker fixture, so named `load`, `scale-in`, and `scale-out`
  phases can run without external wrapper scripts. The Docker
...
- Validate rack-aware routing and auto-balancer decisions under load. Status:
  ...
  target-owner Produce/Fetch after reassignment, and a required fixture-backed
  `load` phase.
  ...
  Live Docker load/scale orchestration hooks, built-in fixture selection, and
  required-phase validation are now pinned by `test-e2e`; broader CI execution
  across real scale-in/out/load environments remains.
...
- Add chaos tests for SIGKILL, network partition, S3 outage, clock skew, and
  ...
  `ZMQ_E2E_LOAD_SCALE_USE_FIXTURE=1` for the built-in Docker fixture. Those
  hooks now receive the active topic plus broker, controller, metrics,
  container, and MinIO context, with deterministic default-suite self-test
  coverage.
