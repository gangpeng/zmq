...
| Tests | Improving. Unit/integration tests run under Zig 0.16. Controller, broker registry, and metadata client tests are included in the default suite. `zig build bench` now exercises repeatable local produce/fetch, S3 WAL request-volume, recovery-time, and bounded memory-growth performance gates. Gated `test-client-matrix` can enforce required version, secured-client, and negative-security profile coverage; gated `test-chaos` now covers broker SIGKILL/local-WAL restart, slow/partial client frames, far-future client timestamps, sync S3 WAL outage fail-closed behavior, CI-provided network-partition hooks, and CI-provided live-S3 outage/heal hooks; `test-s3-provider-matrix` can enforce required live provider, outage, pagination, and multipart-edge profile coverage; gated `test-kraft-failover` can now run CI-provided controller/broker network-partition hooks or scheduled `ZMQ_KRAFT_NETWORK_MATRIX` phases before failover/restart sequencing; and gated `test-e2e` exposes the Docker three-node combined-mode cluster suite with default self-test coverage. Deterministic fixture self-tests run in the default suite. | Add protocol golden, broader multi-node e2e, broader multi-broker chaos, comparative perf, and expanded client matrix gates. |

Latest default-suite observability tranche: JMX-compatible request
queue/local/remote/response timing counters and broker-topic
messages-in/rejected-bytes counters are registered and emitted from real request
and Produce paths. Checked-in Grafana and Prometheus fixtures now reference
these metrics, including rejected-produce-bytes and local-request-time alerts,
and default readiness tests fail if the artifacts drift from the registered
metric corpus.
...
- Add chaos tests for SIGKILL, network partition, S3 outage, clock skew, and
  ...
  required ListObjectsV2 pagination profiles with
