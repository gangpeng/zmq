...
| S3 WAL | Partial. Sync durability path exists and failed uploads are not acknowledged. Filesystem WAL produces now fsync before ack, advance HW on durable write, and replay after local broker restart. Flushed S3 WAL objects can rebuild stream-set metadata when the local object snapshot is missing, including paginated ListObjectsV2 responses. S3 WAL object upload has bounded retry for transient put failures. | S3 crash/restart recovery, idempotent retry, fencing, provider matrix, and fault injection. |
...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  handle paginated ListObjectsV2 recovery; S3 WAL object upload now has bounded
  transient-failure retry. Remaining durability work is crash/fault recovery
  against quorum/controller metadata, idempotent multipart retry, and provider
