...
- Validate rack-aware routing and auto-balancer decisions under load. Status:
  ...
  target exclusion, scale-in leader movement, and simulated post-plan load convergence; reassignment APIs can
  ...
  fence local Produce/Fetch when ownership is non-local. Live reassignment
  execution, controller-driven orchestration, and live load/scale gates remain.
