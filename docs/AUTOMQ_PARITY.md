...
- AutoMQ extension API keys 501-519 and 600-602 are broker-dispatched and
  ...
  committed Raft metadata records when a Raft state is attached, reject
  non-leader mutations, and still retain local snapshot compatibility for
  single-node mode. Manifest and partition snapshot APIs are read-side views.
...
| AutoMQ extension APIs | Implemented locally with quorum-backed controller metadata starting. Keys 501-519 and 600-602 dispatch through generated schemas; stream/object APIs mutate ObjectManager; KV/node/router/license/node-id/group mutations can be backed by committed Raft records and fail closed on attached non-leaders. | Finish ObjectManager quorum records/snapshots, multi-node failover, and client compatibility fixtures. |
...
- Controller-style KV/node/router/license/node-id/group mutations now have a
  committed Raft record path with replay and non-leader fencing. Remaining gap:
  move stream/object metadata snapshots and Raft snapshot compaction behind
  quorum metadata and add multi-node compatibility tests.
