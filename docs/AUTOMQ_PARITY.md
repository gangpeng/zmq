...
directory, controller offline partition, and preferred-replica imbalance
...
- Add compatibility runs with Java, librdkafka, Go, Python, and Kafka CLI
  ...
  OAuth validation now accepts provider-style array `aud` claims and rejects
  future `nbf` tokens before principal extraction.
...
- Add performance baselines for produce/fetch throughput, p99 latency, S3
  ...
  The local benchmark gate and live-MinIO benchmark gate have both been
