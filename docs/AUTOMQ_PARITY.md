...
| P1 | Multi-node KRaft and broker lifecycle | Broker registration, heartbeat, fencing/unfencing, controller failover, rolling restart, and leader epoch behavior are covered by three-node tests. | In progress. Controller Vote, BeginQuorumEpoch, EndQuorumEpoch, DescribeQuorum, BrokerRegistration, BrokerHeartbeat, AllocateProducerIds, AddRaftVoter, and RemoveRaftVoter now use generated schemas on the controller port; controller tests are part of the default Zig test suite. Gated `test-kraft-failover` now starts three controller-only processes, kills the discovered leader, and verifies surviving controllers elect a replacement. Broker lifecycle/fencing coverage remains. |
...
- Replace single-node no-op inter-broker handlers with real controller-backed
  ...
  `test-kraft-failover` step covers three controller-only processes, controller
  leader discovery, leader kill, and replacement leader convergence.
