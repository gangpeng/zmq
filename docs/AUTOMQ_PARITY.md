...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  snapshot writes fail, InitProducerId, AddPartitionsToTxn, AddOffsetsToTxn, and
  EndTxn now restore the previous transaction snapshot when their coordinator
  mutations cannot be written to shared storage, and broker tick now enforces
  each group's configured session timeout instead of a global timeout;
