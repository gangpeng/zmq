...
- Status: Source-level protocol audit tests now parse the generated index and
  ...
  state, restore that state across local broker restart, and roll back local
  assignment visibility when snapshot persistence fails.
...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  configs, and finalized feature snapshots are now written to
