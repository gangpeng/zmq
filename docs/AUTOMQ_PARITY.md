...
- Controller-style KV/node/router/license/node-id/group mutations now have a
  ...
  module, Python interpreter, and semantic-suite overrides so CI can run
  explicit client/library version sets. `ZMQ_CLIENT_MATRIX_SEMANTICS` can now
  require admin, consumer-group, and transactional real-client probes in
  addition to default metadata/produce/fetch/offset checks. Remaining gap:
  broaden rebalance and security semantic client fixtures.
...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  named S3-compatible provider profiles. S3 request signing now omits default
  HTTP/HTTPS ports from the canonical Host header for AWS-style providers and
  preserves explicit custom ports. Real-client ListObjectsV2 pagination now
  follows continuation tokens and fails closed when a provider marks a page
  truncated without returning a token. Remaining durability work is
...
- Add compatibility runs with Java, librdkafka, Go, Python, and Kafka CLI
  ...
  Python interpreters, tool lists, semantic suites, and bootstrap endpoints.
  `ZMQ_CLIENT_MATRIX_SEMANTICS` can require real-client admin topic/config
  operations, consumer-group list/describe or group reads, and transactional
  produce paths for confluent-kafka and Java kafka-clients. Remaining work:
  add deeper rebalance and ACL/security semantic client fixtures.
