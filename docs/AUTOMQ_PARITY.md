...
- Status: local partition offset/HW/LSO snapshots now reload across restart and
  ...
  snapshot cannot be written; CreateTopics/CreatePartitions now roll back topic
  or partition-count visibility and return storage errors when their shared
  topic snapshot cannot be written; local metadata snapshots now fsync before
