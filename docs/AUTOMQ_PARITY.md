...
- Validate rack-aware routing and auto-balancer decisions under load. Status:
  ...
  retain/list/cancel ongoing state, apply target ownership into local failover
  metadata, restore that ownership after restart/replacement replay, expose the
  owner through metadata APIs, and fence local Produce/Fetch when ownership is
  non-local. Controller-backed reassignment orchestration and live load/scale
  gates remain.
